...
- **Specialist KB Bootstrap** — generated specialist agents are born with a KB, and light KBs get promoted when reuse proves the domain matters:
  - `specialist-autoprovision` sub-flow extension: an agent gap in a domain absent from `_index.yaml` declares `kb_domains: [domain]` and appends a KB-creation task (`@kb-architect`, light single-pass on its own frontmatter model — never the main-loop model) dependency-ordered before the specialist's first delegated task; governed by the existing Gate B, no new gates
  - Binding KB-task contract: `genai`-shaped output, `_templates/` structure, structural claims (versions, API signatures, config defaults, code examples) must cite official-docs URLs fetched during the pass, provenance header on every file (`auto-generated`, date, sources, confidence, review note), additive `_index.yaml` registration, blocking hygiene grep
  - Revert path: a KB task that exhausts Gate B retries reverts the agent to `kb_domains: []`, removes partial output, regenerates the router, records WARN — the run never aborts because of a KB task
  - Promotion on reuse (`sdd-build` hook): delegating to an agent whose KB `index.md` reads `Provenance: auto-generated` appends the `--validated` upgrade (`kb-build`) as the run's final best-effort task — max 1 upgrade per run (drip promotion), further candidates deferred with a provenance row
- **Specialist Autoprovision** — design-time specialist-gap detection with a JIT provisioning sub-flow, so intention-driven runs never silently degrade to `(general)` execution when no specialist covers a domain:
  - `specialist-autoprovision` skill (distributed) — citation-based match semantics (a domain resolves only against a citable `routing.json` agent entry or covering skill; no numeric confidence scores), the provisioning sub-flow (component-model layer gate → `create-agent`/`create-skill` SOP authoring → blocking public-repo hygiene grep → mandatory router regeneration + citation verification → deferred checklist items scheduled as manifest build tasks), the supervised/autonomous conduct fork (at most one question, only at the layer decision), provenance row shape, and loud degradation rules (falls back to `(general)` + WARN only when the sensor itself is unavailable — never silently)
  - `sdd-design` Step 4.5 hook — every manifest row must carry a citation before the manifest finalizes; quality gate extended accordingly
  - `sdd-build` delegation safety net — an `@agent` unresolvable at delegation time (design→build drift) enters the same sub-flow instead of failing
  - `sdd-autopilot` Gate P — proceed / retry (1 per gap, regenerate with violations in context) / abort-with-gap-report; sensor unavailability → VISIBLE SKIP + status-quo fallback, never an assumed PASS; provisioning conduct row (autonomous layer fork assumes skill + thin executor, records `[ASSUMED]`)
