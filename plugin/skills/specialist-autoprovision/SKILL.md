...
When the layer is AGENT, check the domain against `${CLAUDE_PLUGIN_ROOT}/kb/_index.yaml`:
...
|---|------|--------|---------|-------|--------------|
| N | `${CLAUDE_PLUGIN_ROOT}/kb/{domain}/` | Create | Bootstrap KB (light + structural-claim validation, genai shape) | @kb-architect | None |
...
- Shape: imitate `${CLAUDE_PLUGIN_ROOT}/kb/genai/` (index.md, quick-reference.md, 3-4 concepts/, 3-4 patterns/)
- Structure: binding templates in `${CLAUDE_PLUGIN_ROOT}/kb/_templates/`
...
- Register the domain ADDITIVELY in `${CLAUDE_PLUGIN_ROOT}/kb/_index.yaml` — never modify other entries.
...
- KB machinery (reused verbatim): `${CLAUDE_PLUGIN_ROOT}/agents/architect/kb-architect.md` · `/create-kb` (light + `--validated`) · `${CLAUDE_PLUGIN_ROOT}/skills/kb-build/SKILL.md` · `${CLAUDE_PLUGIN_ROOT}/kb/_templates/` · `${CLAUDE_PLUGIN_ROOT}/kb/genai/` (few-shot shape)
