...
2. Which KB domains apply? (list available from ${CLAUDE_PLUGIN_ROOT}/kb/)
...
| Structure | `${CLAUDE_PLUGIN_ROOT}/sdd/templates/DEFINE_TEMPLATE.md` — the template owns the document format; follow it rather than re-creating sections here |
...
Completing Define carries two status obligations, per `${CLAUDE_PLUGIN_ROOT}/sdd/architecture/WORKFLOW_CONTRACTS.yaml` (`status_transitions` and `update_rules`). Skipping them leaves stale "Ready for X" statuses that break the workflow.
...
```bash
${CLAUDE_PLUGIN_ROOT}/tools/spec-linter/spec-lint .claude/sdd/features/DEFINE_{FEATURE_NAME}.md --phase define \
